...
- Never convert a computed quota or token count to `int` with a bare cast like `int(float64(quota) * ratio)`, `int(math.Round(...))` on unbounded input, or `int(decimal.IntPart())`. All quota rounding/conversion is centralized in `common/quota_math.go`; use those helpers: `common.QuotaFromFloat` (truncating) for float products, `common.QuotaRound` (half-away-from-zero) where rounding is intended, and `common.QuotaFromDecimal` for decimal products. `billingexpr.QuotaRound` delegates to `common.QuotaRound`. Do not reintroduce local conversion helpers or bare casts. These saturation bounds are int32 because they cap a *single request's charge*, not because of the column width: quota columns are `bigint`, since GORM maps a Go `int` field to 64 bits on every supported database (a `gorm:"type:int"` tag does not narrow it — `int` there is the `schema.Int` kind, not a native SQL type). Every clamp/NaN fallback is logged via `common.SysError` since a single request should never approach those bounds.
- Accumulated balances are a different scale from per-request charges and MUST NOT be pushed through the int32 helpers — doing so caps a wallet at about 4294 display units at the default `QuotaPerUnit`. Wallet balances, token allowances, and top-up credit are bounded by `common.MaxWalletQuota` (2^53-1, the largest integer a frontend JSON number represents exactly) and converted with `common.QuotaFromDecimalWalletStrict`, which rejects out-of-range input instead of saturating, because a silently truncated balance is worse than a rejected request. Every path that credits a balance enforces this bound: top-up validation and per-provider settlement (`controller/topup.go`, `model/topup.go`), admin `add_quota` (`controller/user.go`), redemption (`model/redemption.go`), and token allowance (`controller/token.go`). Credit paths that can race enforce it as a single conditional `UPDATE` so the predicate and the increment cannot be split (see `creditTopUpQuota`). Do not put this bound inside `model.IncreaseUserQuota`: it is shared with refund paths, where a failed write would cost the user quota.
