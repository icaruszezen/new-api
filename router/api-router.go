...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		// 渠道监控状态页对未登录访客公开。游客走 CM（IP）桶，登录用户走 CMU（user）桶，
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.GET("/invite", controller.GetInviteOverview)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/invite/stats", controller.GetInviteAdminStats)
			}
		}
		...
		registerChannelMonitoringRoutes(apiRouter)
		...
		dataRoute.GET("/summary", middleware.UserAuth(), controller.GetUserQuotaDataSummary)
